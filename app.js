...
app.use("/admin", isAdmin, adminAuthRoutes);
...
app.post("/create-product", upload.array("images", 5), async (req, res) => {
  try {
    ...
    const product = new Product({ ...productData, img: imageUrls }); // Save image URLs
    await product.save();
...
app.post("/place-order", async (req, res) => {
  ...
  const shippingCost = totalAmount > 499 ? 0 : 50; // Free delivery above ₹499
  if (totalAmount > 499) {
